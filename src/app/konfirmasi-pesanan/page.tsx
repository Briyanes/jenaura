...
export default async function OrderConfirmationPage({
  ...
  searchParams: Promise<{ id?: string; resultCode?: string }>
}) {
  const { id, resultCode } = await searchParams
  const orderId = id || 'PENDING'
  const isPaid = resultCode === '00'
  const isCod = !resultCode // no resultCode means COD flow
  ...
  const nextStepsCod = [
    ...
    'Paket sampai di depan pintu — bayar ke kurir! 📦',
  ]

  const nextStepsPaid = [
    'Pembayaran kamu sedang diverifikasi otomatis',
    'Pesanan diproses setelah pembayaran terkonfirmasi',
    'Kamu menerima nomor resi pengiriman via WhatsApp',
      ...
      <div className="relative w-full max-w-md">
        <div className="bg-white/[0.07] backdrop-blur-xl border border-white/[0.12] rounded-3xl p-8 sm:p-10 text-center shadow-2xl">
          {isPaid ? (
            ...
            <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-jena-gold/15 border border-jena-gold/25 flex items-center justify-center">
              {isCod ? (
                <CheckCircle size={32} className="text-jena-gold" strokeWidth={1.5} />
              ) : (
                <Clock size={32} className="text-jena-gold" strokeWidth={1.5} />
              )}
            </div>
          )}

          <h1 className="font-display text-2xl sm:text-3xl text-white font-bold mb-3">
            {isPaid ? 'Pembayaran Berhasil! 🎉' : isCod ? 'Pesanan Diterima! 🎉' : 'Menunggu Pembayaran...'}
          </h1>
          <p className="text-base text-white/40 mb-7 leading-relaxed">
            {isPaid
              ? 'Terima kasih! Pembayaran kamu berhasil dikonfirmasi. Kami akan segera memproses pesananmu.'
              : isCod
              ? 'Terima kasih! Pesanan COD kamu sudah kami terima. Kami akan segera memproses.'
              : 'Pesanan kamu sudah dibuat. Selesaikan pembayaran agar pesanan segera diproses.'}
          </p>
          ...
          <div className="bg-white/[0.06] border border-white/[0.08] rounded-2xl px-5 py-4 mb-7">
            ...
            <p className="text-base font-bold text-jena-gold font-mono tracking-wider">{orderId}</p>
          </div>

          <div className="text-left mb-8">
            <p className="text-xs font-bold text-white/40 uppercase tracking-widest mb-3">Yang akan terjadi selanjutnya:</p>
            <ol className="space-y-2.5">
              {(isPaid || isCod ? (isPaid ? nextStepsPaid : nextStepsCod) : nextStepsPaid).map((item, i) => (
